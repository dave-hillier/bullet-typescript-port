...
export class btBoxShape extends btConvexInternalShape {
    ...
    localGetSupportingVertex(vec) {
        ...
        return new btVector3(btFsels(vec.x(), halfExtentsWithMargin.x(), -halfExtentsWithMargin.x()), btFsels(vec.y(), halfExtentsWithMargin.y(), -halfExtentsWithMargin.y()), btFsels(vec.z(), halfExtentsWithMargin.z(), -halfExtentsWithMargin.z()));
    }
    /**
    ...
    localGetSupportingVertexWithoutMargin(vec) {
        const halfExtents = this.getHalfExtentsWithoutMargin();
        return new btVector3(btFsels(vec.x(), halfExtents.x(), -halfExtents.x()), btFsels(vec.y(), halfExtents.y(), -halfExtents.y()), btFsels(vec.z(), halfExtents.z(), -halfExtents.z()));
    }
    /**
    ...
    batchedUnitVectorGetSupportingVertexWithoutMargin(vectors, supportVerticesOut, numVectors) {
        ...
        for (let i = 0; i < numVectors; i++) {
            const vec = vectors[i];
            supportVerticesOut[i] = new btVector3(btFsels(vec.x(), halfExtents.x(), -halfExtents.x()), btFsels(vec.y(), halfExtents.y(), -halfExtents.y()), btFsels(vec.z(), halfExtents.z(), -halfExtents.z()));
        }
    }
    /**
    ...
    setLocalScaling(scaling) {
        const oldMargin = new btVector3(this.getMargin(), this.getMargin(), this.getMargin());
        const implicitShapeDimensionsWithMargin = this.implicitShapeDimensions.add(oldMargin);
        const unScaledImplicitShapeDimensionsWithMargin = new btVector3(implicitShapeDimensionsWithMargin.x() / this.localScaling.x(), implicitShapeDimensionsWithMargin.y() / this.localScaling.y(), implicitShapeDimensionsWithMargin.z() / this.localScaling.z());
        super.setLocalScaling(scaling);
        this.implicitShapeDimensions = new btVector3(unScaledImplicitShapeDimensionsWithMargin.x() * this.localScaling.x(), unScaledImplicitShapeDimensionsWithMargin.y() * this.localScaling.y(), unScaledImplicitShapeDimensionsWithMargin.z() * this.localScaling.z()).subtract(oldMargin);
    }
    /**
