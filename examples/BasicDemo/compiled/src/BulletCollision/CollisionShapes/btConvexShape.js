 ...
 */
export function convexHullSupport(localDirOrg, points, _numPoints, localScaling) {
    const vec = new btVector3(localDirOrg.x() * localScaling.x(), localDirOrg.y() * localScaling.y(), localDirOrg.z() * localScaling.z());
    ...
    return new btVector3(points[ptIndex].x() * localScaling.x(), points[ptIndex].y() * localScaling.y(), points[ptIndex].z() * localScaling.z());
}
