 ...
 */
export function AabbExpand(aabbMin, aabbMax, expansionMin, expansionMax) {
 ...
 */
export function TestPointAgainstAabb2(aabbMin1, aabbMax1, point) {
    let overlap = true;
 ...
 */
export function TestAabbAgainstAabb2(aabbMin1, aabbMax1, aabbMin2, aabbMax2) {
    let overlap = true;
 ...
 */
export function TestTriangleAgainstAabb2(vertices, aabbMin, aabbMax) {
    const p1 = vertices[0];
    const p2 = vertices[1];
    const p3 = vertices[2];
    if (btMin(btMin(p1.getX(), p2.getX()), p3.getX()) > aabbMax.getX())
        return false;
    if (btMax(btMax(p1.getX(), p2.getX()), p3.getX()) < aabbMin.getX())
        return false;
    if (btMin(btMin(p1.getZ(), p2.getZ()), p3.getZ()) > aabbMax.getZ())
        return false;
    if (btMax(btMax(p1.getZ(), p2.getZ()), p3.getZ()) < aabbMin.getZ())
        return false;
    if (btMin(btMin(p1.getY(), p2.getY()), p3.getY()) > aabbMax.getY())
        return false;
    if (btMax(btMax(p1.getY(), p2.getY()), p3.getY()) < aabbMin.getY())
        return false;
    return true;
}
/**
 ...
 */
export function btOutcode(p, halfExtent) {
 ...
 */
export function btRayAabb2(rayFrom, rayInvDirection, raySign, bounds, tmin, lambda_min, lambda_max) {
 ...
 * @returns True if the ray intersects the AABB
 */
export function btRayAabb(rayFrom, rayTo, aabbMin, aabbMax, param, normal) {
 ...
 */
export function btTransformAabb(halfExtents, margin, t, aabbMinOut, aabbMaxOut) {
 ...
 * @param aabbMaxOut Output maximum bounds
 */
export function btTransformAabbOverload(localAabbMin, localAabbMax, margin, trans, aabbMinOut, aabbMaxOut) {
 ...
 */
export function testQuantizedAabbAgainstQuantizedAabb(aabbMin1, aabbMax1, aabbMin2, aabbMax2) {
