...
// Forward declaration for collision object interface
export interface btCollisionObject {
    // Minimal interface - will be expanded when btCollisionObject is ported
}

/** maximum contact breaking and merging threshold */
export let gContactBreakingThreshold = 0.02;

/** Contact callback function types */
export type ContactDestroyedCallback = (userPersistentData: any) => boolean;
export type ContactProcessedCallback = (cp: btManifoldPoint, body0: any, body1: any) => boolean;
export type ContactStartedCallback = (manifold: btPersistentManifold) => void;
export type ContactEndedCallback = (manifold: btPersistentManifold) => void;

/** Global contact callbacks - using a mutable object to allow test modification */
export const globalContactCallbacks = {
    gContactDestroyedCallback: null as ContactDestroyedCallback | null,
    gContactProcessedCallback: null as ContactProcessedCallback | null,
    gContactStartedCallback: null as ContactStartedCallback | null,
    gContactEndedCallback: null as ContactEndedCallback | null
};
...
export class btPersistentManifold extends btTypedObject {
    private m_pointCache: btManifoldPoint[];

    /** this two body pointers can point to the physics rigidbody class. */
    private m_body0: btCollisionObject | null;
    private m_body1: btCollisionObject | null;

    private m_cachedPoints: number;

    private m_contactBreakingThreshold: number;
    private m_contactProcessingThreshold: number;
    ...
    constructor();
    constructor(body0: btCollisionObject | null, body1: btCollisionObject | null, unused?: number, contactBreakingThreshold?: number, contactProcessingThreshold?: number);
    constructor(body0?: btCollisionObject | null, body1?: btCollisionObject | null, _unused?: number, contactBreakingThreshold?: number, contactProcessingThreshold?: number) {
    ...
    getBody0(): btCollisionObject | null {
        return this.m_body0;
    }

    getBody1(): btCollisionObject | null {
        return this.m_body1;
    }

    setBodies(body0: btCollisionObject | null, body1: btCollisionObject | null): void {
        this.m_body0 = body0;
        this.m_body1 = body1;
    }
    ...
    addManifoldPoint(newPoint: btManifoldPoint, _isPredictive: boolean = false): number {
    ...
    private sortCachedPoints(pt: btManifoldPoint): number {
    ...
    // private findContactPoint(_unUsed: btManifoldPoint[] | null, _numUnused: number, pt: btManifoldPoint): number {
    //     // Simple implementation for now - could be optimized further
    //     return this.getCacheEntry(pt);
    // }
}