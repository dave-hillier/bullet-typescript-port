...
import { btConstraintSolver, btConstraintSolverType, btTypedConstraint, btIDebugDraw } from './btConstraintSolver';
...
// Forward declarations for solver-specific types that would need full implementation later
interface btSolverBody {
  // Placeholder for solver body data
}

interface btSolverConstraint {
  // Placeholder for solver constraint data  
}

export interface btSolverAnalyticsData {
  m_numSolverCalls: number;
  m_numIterationsUsed: number;
  m_remainingLeastSquaresResidual: number;
  m_islandId: number;
  m_numBodies: number;
  m_numContactManifolds: number;
}
